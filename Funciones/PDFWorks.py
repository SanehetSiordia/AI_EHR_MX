...
def extraccionPDFNotaInicial(rutaPDF,diccEstandar):
    ...
    print('\n\tprocesando nota clinica inicial'.upper())
    ...
    print('Diccionario Inicial: ', diccEstandar)
    ...
    for iterador in range(sizeList):
        TextSplit = []
        print('\ntexto buscado: ' + str(listTextoBuscado[iterador]) + '\nformato buscado: ' + str(
            listFormatoBuscado[iterador]))
        ...
        try:
            if listTextoBuscado[iterador]=="\\nDIAGNOSTICOS O PROBLEMAS CLINICOS".lower() and NotaControl==False:
                ...
                print('COMPARAR CON CODIGOS CIE10')
        ...
        try:
            if listTextoBuscado[iterador]=="Fecha de Clasificacion:".lower():
                ...
                print('\tFecha Nota:\t', reSearch.date())
            else:None
        except:
            print('\n\tError con deteccion de la fecha'.upper())
            errorDate=True
        ...
        print(str(listaValorDiccionario[iterador]))
        ...
        if ((str(listTextoBuscado[iterador]))=="RESUMEN DEL INTERROGATORIO".lower()) and (reSearch==None):
            print('\n\tError con lectura de campos correctamente'.upper())
            errorExtracionNSS=(str(diccEstandar['nss']))

        elif errorDate:
            print('\n\tError con lectura de fecha correctamente'.upper())
            errorExtracionNSS=(str(diccEstandar['nss']))

    print('Diccionario Final: ',diccEstandar,'\n')
...
def extraccionPDFLaboratorio(rutaPDF,listFormatos,listDiccionario):
    ...
    print('\n\t\tINICIO extracion Texto Laboratorio')
    pagesText=''
    text=''
    textSplit=''
    ...
    f.close()
    #__________________________________________________________________________________
    print('\n\t\tFIN extracion Texto Laboratorio')
    #+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    try:
        ...
        if nss==dictGeneral['nss']:
            print('\tNSS COINDICEN CORRECTAMENTE')
            #conversion de string a double
            nss=int(nss)
            print('\tNSS:\t', nss)
            dictGeneral['nss']=nss
        else:
            print('\tERROR... NSS NO COINCIDEN ENTRE SI')
            errorExtracionNSS = str(dictGeneral['nss'])
    except:
        print('\tNo se cumplio condicion de busqueda: NSS')
        errorExtracionNSS=str(dictGeneral['nss'])
    ...
    try:
        ...
        print('\tFolio Orden:\t',folioOrden)
        dictGeneral['folio_orden'] = folioOrden
    except:
        print('\tNo se cumplio condicion de busqueda: FOLIO ORDEN')
        errorExtracionNSS=str(dictGeneral['nss'])
    ...
    try:
        ...
        print('\tFecha Orden:\t', fechaOrden.date())
    ...
    try:
        ...
        print('\tEstudios Encontrados:\t',listEstudiosLabs)
        print('\ttamaño textSplit: ' + str(len(textSplit)) + ' - tamaño listEstudiosLabs: ' + str(len(listEstudiosLabs)))
    except:
    ...
    if errorExtracionNSS=='':
        ...
        for i in range(1, len(textSplit)):
            ...
            try:
                ...
                print('\t\tTamaño:', len(listDeterminacion), '-Resultado: ', listDeterminacion)
            ...
            try:
                ...
                print('\t\tTamaño:', len(listResultados), '-Resultado: ', listResultados)
            ...
            try:
                ...
                print('\t\tTamaño:', len(listUnidades), '-Resultado: ', listUnidades)
            ...
            try:
                ...
                print('\t\tTamaño:', len(listValoresNormales), '-Resultado: ', listValoresNormales)
            ...
            if len(listDeterminacion)==len(listResultados)  and \
                ...
                if len(listDeterminacion)>0 and len(listUnidades)>0 and len(listResultados)>0 and len(listValoresNormales)>0:
                    ...
                    print('SE INOGRA ESTUDIO: ' + str(listEstudiosLabs[j]) + ' POR AUSENCIA DE RESULTADOS')

            else:
                dictEstudios.pop(str(listEstudiosLabs[j]))
                print('SE INOGRA ESTUDIO: '+str(listEstudiosLabs[j])+' POR PROBLEMAS DE LECTURA')
...
def extraccionPDFNotaFinal(rutaPDF,diccEstandar):
    ...
    print('\n\tprocesando nota clinica final'.upper())
    ...
    for iterador in range(sizeList):
        ...
        try:
            if listTextoBuscado[iterador]=="Fecha de Clasificacion:".lower():
                #Conversion a formato YYYY-MM-DD
                reSearch= datetime.datetime.strptime(reSearch, '%d/%m/%Y')
                print('\tFecha Egreso:\t', reSearch.date())
            elif listTextoBuscado[iterador]=="\\nFECHA DE INGRESO".lower():
                #Conversion a formato YYYY-MM-DD
                reSearch= datetime.datetime.strptime(reSearch, '%d/%m/%Y')
                print('\tFecha Ingreso:\t', reSearch.date())
            else:None
...
def pdfsOrganizar(rutaOrigen,rutasNotaClase,rutasNotasClaseDX,rutaPDF):
    ...
    if typePDF==1:
        ...
        try:
            ...
            print('PDF repetido mas de dos veces')
            PDFrep+=1
    ...
    if len(archivos_en_BD)>0:
        ...
        for valor in PDFsListNSSFloat:
            ...
            if valor in archivos_en_BD:
                print('\tValor: ',valor,' Existente en Base de datos')
    ...
    if len(contenidoCarpeta)>0:
        ...
        for fichero in contenidoCarpeta:
            if os.path.isfile(os.path.join(rutaCarpetaPDFs, fichero)) and fichero.endswith('.pdf'):
                ...
                print('Diccionario particular del PDF para nota inicial: \n',datosNotaInicial)
        ...
        if listaErroresNSS:
            ...
            print('\tNSS con problemas de extracion:\n\t'.upper(),listaErroresNSS)
...
def pdfsNotaEgreso(rutaOrigen,rutaCarpetaPDFs,rutaCarpetaErrorPDFS,archivos_en_BD):
    print('\n\tEvaluacion de Notas Finales/Egreso'.upper())
    resultado={}
    PDFsListNSSActuales=[]
    PDFsListNSSFloat=[]
    indexListNSS=[]
    ...
    if len(contenidoCarpetaEgreso[0])>0:
        ...
        for fichero in contenidoCarpetaEgreso[0]:
            ...
            if os.path.isfile(os.path.join(rutaCarpetaNeumonia, fichero)) and fichero.endswith('.pdf'):
                ...
                print('Diccionario particular del PDF para nota Final: \n',datosNotaFinal)
